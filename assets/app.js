...
(() => {
  ...
  const cards = [...section.querySelectorAll('.game-grid a.game-card')];
...
(() => {
  ...
  const cards = [...grids[0].querySelectorAll('a.game-card')];
  if (cards.length < 2 || !cards.some(isFree)) return;
  [...grids[1].querySelectorAll('a.game-card')].forEach(c => grids[0].appendChild(c));
