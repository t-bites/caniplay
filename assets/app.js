...
(() => {
  ...
  function spOpen(svg) {
    ...
    if (bestIdx >= 0) {
      ...
      bestEl.innerHTML = `${tag} ${spFull(bestVal, metric)} · Range ${spFmt(minVal, metric)}–${spFmt(maxVal, metric)}${spanTag}`;
    }
...
/* #200 game-sales + #201 deals 促销/免费卡 wishlist ♥ 快捷收藏钮（GG.deals/Steam 式，承接 #199 新发现）
   ...
   本 IIFE 负责画初始收藏态 + 点击切换（写 caniplay_wl 共用集合 + meta）+ 跨 tab 同步。 */
(() => {
  if (!document.querySelector('.wcard')) return;   // 仅含 ♥ 卡的页（game-sales / deals）
  const wlLoad = () => { try { const a = JSON.parse(localStorage.getItem('caniplay_wl') || '[]'); return Array.isArray(a) ? a.filter(x => typeof x === 'number') : []; } catch (e) { return []; } };
  let wlSet = new Set(wlLoad());
  const wlSave = () => { try { localStorage.setItem('caniplay_wl', JSON.stringify([...wlSet].slice(0, 500))); } catch (e) {} try { window.dispatchEvent(new Event('caniplay:wlsync')); } catch (e) {} };
  const paint = () => {
    document.querySelectorAll('.wcard .wl-btn').forEach(b => {
      const on = wlSet.has(parseInt(b.dataset.a, 10));
      b.classList.toggle('on', on);
      b.setAttribute('aria-pressed', on);
      b.title = on ? 'Remove from wishlist' : 'Add to wishlist';
  ...
  paint();
  document.addEventListener('click', (e) => {
    const btn = e.target.closest('.wcard .wl-btn');
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    ...
    const was = wlSet.has(id);
    const wrap = btn.closest('.wcard');
    const key = 'caniplay_wl_meta';
    const mo = (() => { try { return JSON.parse(localStorage.getItem(key) || '{}'); } catch (er) { return {}; } })();
    if (was) { wlSet.delete(id); delete mo[id]; }
    else {
      wlSet.add(id);
      const card = wrap && wrap.querySelector('.game-card');
      const nm = (wrap && wrap.querySelector('h3')) ? (wrap.querySelector('h3').textContent || '').replace(/\s+/g, ' ').trim().slice(0, 60) : '';
      mo[id] = { n: nm, u: (card && card.getAttribute('href')) || String(id) };
    }
